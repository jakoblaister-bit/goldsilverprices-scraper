"""
push_mgc.py
Scrapes Melbourne Gold Company, deletes existing rows, inserts fresh data.

Run:  python push_mgc.py
Disable in push_all.py: comment out the Melbourne Gold Company entry in the dealers list.
"""

import json, urllib.request, urllib.error
from datetime import datetime, timezone
from scrape_mgc import fetch_products, DEALER
...
def push():
    ...
    # Gate: only insert rows with a valid buy_price. Sell-only rows (N/A buy)
    # are dropped here — a null buy_price must never reach the DB.
    before = len(scraped)
    scraped = [r for r in scraped if r.get("buy_price") and r["buy_price"] > 0]
    print(f"  {len(scraped)} with buy_price > 0  ({before - len(scraped)} sell-only dropped)")
