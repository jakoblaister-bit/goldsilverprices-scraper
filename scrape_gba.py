"""
scrape_gba.py
Fetches live buy + sell prices from goldbullionaustralia.com.au/live-charts-prices/
"""

import re, html as htmllib, urllib.request

DEALER     = "Gold Bullion Australia"
BASE       = "https://www.goldbullionaustralia.com.au"
...
EXCLUDE = [
    ...
    "blister pack",
    "platinum",
    "random dates/design",   # "Perth Mint 2oz Gold Coin – Random Dates/Design"
]
...
def coin_type_for(name):
    ...
    # "Horse Rectangular" on GBA means a rectangular shaped lunar coin
    if "horse rectangular" in lo:
        return "Lunar Horse"
    ...
    # "Dragon Rectangular" — treat as Dragon coin type
    if "dragon" in lo:
        return "Dragon"
...
def classify_bar(name):
    ...
    if "assorted" in lo or "generic" in lo:
...
def classify(name):
    ...
    if "gold" in lo and "silver" not in lo:
        metal = "gold"
    elif "silver" in lo and "gold" not in lo:
        metal = "silver"
    elif "gold" in lo:
        metal = "gold"
    else:
